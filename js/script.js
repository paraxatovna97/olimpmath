(function() {
    ...
    const IDLE_BREATH_MIN = 0.98;
    const IDLE_BREATH_MAX = 1.03;
    const IDLE_BREATH_DURATION_MIN = 4000;
    const IDLE_BREATH_DURATION_MAX = 6000;
    const GLOW_PULSE_MIN = 20;
    const GLOW_PULSE_MAX = 35;
    const GLOW_PULSE_SPEED = 0.0015;
    const PHOTON_COUNT = 3;
    const PHOTON_SPEED_MIN = 2000;
    const PHOTON_SPEED_MAX = 3000;

    // ─── NODE HELPER ───
    function buildNode(data, index) {
        const breathDuration = IDLE_BREATH_DURATION_MIN + Math.random() * (IDLE_BREATH_DURATION_MAX - IDLE_BREATH_DURATION_MIN);
        return {
            ...data,
            rx: data.x,
            ry: data.y,
    ...
    function initNodes() {
        ...
        edges = EDGES.map(([from, to]) => {
            ...
            return {
                ...
                photons: Array.from({ length: PHOTON_COUNT }, () => ({
                    progress: Math.random(),
                    speed: PHOTON_SPEED_MIN + Math.random() * (PHOTON_SPEED_MAX - PHOTON_SPEED_MIN),
                })),
            };
        });
    }
    ...
    function getNodeRadius(node) {
        return node.radius * scale * node.currentScale * node.clickScale * node.idleBreath;
    }
    ...
    function drawEdges(time) {
        ...
        for (const edge of edges) {
            ...
            // ─── PHOTONS (traveling light particles) ───
            for (const photon of edge.photons) {
                photon.progress += (1 / photon.speed) * 16;
                if (photon.progress > 1) photon.progress -= 1;

                const pPos = photon.progress;
                const px2 = startX + (endX - startX) * pPos;
                const py2 = startY + (endY - startY) * pPos;

                const dotAlpha = 0.4 + 0.6 * (1 - Math.abs(pPos - 0.5) * 2);
                const dotRadius = 1.5 + 2.5 * dotAlpha * Math.min(scale, 1.2);

                ctx.beginPath();
                ctx.arc(px2, py2, dotRadius, 0, Math.PI * 2);
                ctx.fillStyle = isHovered ?
                    `rgba(79, 70, 229, ${0.8 * dotAlpha})` :
                    `rgba(255, 255, 255, ${0.4 * dotAlpha})`;
                ctx.shadowColor = isHovered ? 'rgba(79, 70, 229, 0.3)' : 'rgba(255, 255, 255, 0.1)';
    ...
    function drawNodes(time) {
        ...
        for (const node of nodes) {
            ...
            const breathProgress = (time / node.breathDuration + node.breathPhase) % 1;
            const breathValue = Math.sin(breathProgress * Math.PI * 2);
            node.idleBreath = 1 + (breathValue * 0.015);
            ...
            // ─── GLOW PULSE ───
            const glowPulse = GLOW_PULSE_MIN + (GLOW_PULSE_MAX - GLOW_PULSE_MIN) *
                (0.5 + 0.5 * Math.sin(time * GLOW_PULSE_SPEED + node.breathPhase));
            ...
            if (finalGlow > 0.01 || isActive) {
                ...
                const glowSize = isActive ? glowPulse * 1.2 : glowPulse;
                const grad = ctx.createRadialGradient(pos.x, pos.y, finalR * 0.3, pos.x, pos.y, glowR + glowSize * 0.3);
                ...
                ctx.beginPath();
                ctx.arc(pos.x, pos.y, glowR + glowSize * 0.3, 0, Math.PI * 2);
                ctx.fill();
                ctx.globalAlpha = 1;
            }
            ...
            if (isActive || finalGlow > 0.1) {
                ...
                ctx.shadowBlur = glowPulse * finalGlow;
            } else {
    ...
    function render(timestamp) {
        ...
        for (const node of nodes) {
            node._parallaxX = currentParallaxX * 15;
            node._parallaxY = currentParallaxY * 15;
        }
