(function() {
    ...
    // ─── NODE DATA ───
    const DOMAINS = [
        { id: 'matematika', label: 'Matematika', x: 0, y: 0, radius: 58, color: '#4F46E5', difficulty: 1,
        mastery: 100 },
    ...
    function drawEdges(time) {
        ...
        for (const edge of edges) {
            ...
            const alpha = Math.min(0.4, 0.15 + 0.25 * progress);
            ...
            if (isHovered) {
                const glowAlpha = 0.2 + 0.25 * breath;
                ctx.strokeStyle = `rgba(79, 70, 229, ${glowAlpha})`;
                ctx.lineWidth = 3 * Math.min(scale, 1.5);
            ...
            if (progress > 0.05 && progress < 0.95) {
                ...
                for (let i = 0; i < dotCount; i++) {
                    ...
                    ctx.fillStyle = isHovered ?
                        `rgba(79, 70, 229, ${0.5 * dotAlpha})` :
                        `rgba(255, 255, 255, ${0.3 * dotAlpha})`;
                    ctx.fill();
                }
            }
        }
    ...
    function drawNodes(time) {
        ...
        for (const node of nodes) {
            ...
            // ─── GLOW (outer) ───
            if (finalGlow > 0.01 || hoveredNode === node) {
                const glowR = finalR * (1 + 0.5 * (hoveredNode === node ? 0.8 : 0.4));
                const glowAlpha = hoveredNode === node ? 0.2 * finalGlow : 0.08 * finalGlow;
            ...
            // Shadow
            ctx.shadowColor = `rgba(79, 70, 229, ${0.12 * finalGlow})`;
            ...
            // ─── INNER GLOW (Apple style) ───
            const innerGlow = ctx.createRadialGradient(
                pos.x - finalR * 0.15, pos.y - finalR * 0.2, 0,
                pos.x, pos.y, finalR
            );
            ...
            // ─── BORDER (Glass edge) ───
            const borderAlpha = 0.04 + 0.1 * finalGlow + (hoveredNode === node ? 0.1 : 0);
    ...
    function drawBackground() {
        ...
        nebula.addColorStop(0, 'rgba(79, 70, 229, 0.03)');
        ...
        nebula2.addColorStop(0, 'rgba(34, 211, 238, 0.02)');
